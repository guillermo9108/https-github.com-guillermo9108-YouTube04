    ...
    try {
      ...
      if (action === 'purchase_vip_instant') {
        ...
        db.transaction(() => {
          ...
          // Credit VIP plan price to the Admin account
          const admin = db.prepare("SELECT id FROM users WHERE role = 'ADMIN' LIMIT 1").get() as any;
          if (admin) {
            db.prepare("UPDATE users SET balance = balance + ? WHERE id = ?").run(plan.price, admin.id);
          }
          
          // Since VIP purchase goes directly to the platform, the entire amount is an adminFee/commission
          db.prepare("INSERT INTO transactions (id, buyerId, amount, type, timestamp, videoTitle, isExternal, adminFee) VALUES (?, ?, ?, 'VIP', ?, ?, 1, ?)")
            .run('txv_'+Date.now(), userId, plan.price, now, plan.name, plan.price);
        })();
        return res.json({ success: true });
      }
      ...
      if (action === 'purchase_category') {
        ...
        db.transaction(() => {
          ...
          // Credit category purchase price to the Admin account
          const admin = db.prepare("SELECT id FROM users WHERE role = 'ADMIN' LIMIT 1").get() as any;
          if (admin) {
            db.prepare("UPDATE users SET balance = balance + ? WHERE id = ?").run(cat.price, admin.id);
          }
          
          // Since category purchase goes directly to the platform, the entire amount is an adminFee/commission and counts as real cash inflow
          db.prepare("INSERT INTO transactions (id, buyerId, amount, adminFee, type, timestamp, videoTitle, isExternal) VALUES (?, ?, ?, ?, 'PURCHASE', ?, ?, 1)")
            .run('txc_'+Date.now(), userId, cat.price, cat.price, Math.floor(Date.now()/1000), "Compra de categoría: " + cat.name);
