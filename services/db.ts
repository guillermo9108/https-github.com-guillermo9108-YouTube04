...
class DBService {
    ...
    public async getVideos(page: number = 0, limit: number = 40, folder: string = '', search: string = '', category: string = '', mediaType: string = 'ALL', sortOrder: string = '', userId: string = '', adminMode: boolean = false, transcodeStatus: string = ''): Promise<VideoPagedResponse> {
        const offset = page * limit;
        const query = `action=get_videos&limit=${limit}&offset=${offset}&folder=${encodeURIComponent(folder)}&search=${encodeURIComponent(search)}&category=${encodeURIComponent(category)}&media_type=${encodeURIComponent(mediaType)}&sort_order=${encodeURIComponent(sortOrder)}&userId=${encodeURIComponent(userId)}${adminMode ? '&admin_mode=1' : ''}${transcodeStatus ? `&transcode_status=${encodeURIComponent(transcodeStatus)}` : ''}`;
    ...
    public async getAllVideos(adminMode: boolean = false, transcodeStatus: string = ''): Promise<Video[]> { 
        const res = await this.getVideos(0, 10000, '', '', '', 'ALL', '', '', adminMode, transcodeStatus);
        return res.videos;
    }
    ...
    public async createGroup(userId: string, name: string, description?: string, isPrivate?: boolean, coverUrl?: string, allowUpload?: boolean, isSeries?: boolean): Promise<any> {
        return this.request<any>('action=group_create', {
            method: 'POST',
            body: JSON.stringify({ userId, name, description, isPrivate, coverUrl, allowUpload: allowUpload ? 1 : 0, isSeries: isSeries ? 1 : 0 })
        });
    }

    public async editGroup(userId: string, folderPath: string, name?: string, description?: string, isPrivate?: boolean, coverUrl?: string, isUnified?: boolean, allowUpload?: boolean, isSeries?: boolean): Promise<any> {
        return this.request<any>('action=group_edit', {
            method: 'POST',
            body: JSON.stringify({ userId, folderPath, name, description, isPrivate, coverUrl, isUnified, allowUpload: allowUpload ? 1 : 0, isSeries: isSeries ? 1 : 0 })
        });
    }
