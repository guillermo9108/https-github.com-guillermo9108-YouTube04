...
function admin_handle_vip_request($pdo, $input) {
    ...
    try {
        ...
        $pdo->commit(); respond(true);
    } catch (Throwable $e) { $pdo->rollBack(); respond(false, null, $e->getMessage()); }
}
...
function admin_repair_db($pdo, $input) {
    ...
    // Fix específico para lastDeviceId que suele truncarse
    try {
        $pdo->exec("ALTER TABLE users MODIFY COLUMN lastDeviceId TEXT");
    } catch (\Exception $e) {}
